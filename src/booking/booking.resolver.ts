...
import { Resolver, Query, Args, Mutation, Float, Context, Int } from '@nestjs/graphql';
import { BookingService } from './booking.service';
import { Booking, BookingResponse } from './booking.schema';
...
export class BookingResolver {
  ...
  @Mutation(() => Booking)
  async verifyAndInvalidateBooking(
    @Args('ticket_no') ticket_no: string,
  ): Promise<Booking> {
  ...
  async bookings(
    ...
    const [bookings, totalBookings] = await Promise.all([
      this.bookingService.findAll({ limit, offset }),
      this.bookingService.countAll(),
    ]);
  ...
  @Query(() => [Booking], { name: 'searchBookings' })
  async searchBookings(
    @Args('searchTerm', { type: () => String }) searchTerm: string
  ): Promise<Booking[]> {
    return this.bookingService.searchBookings(searchTerm);
  }
